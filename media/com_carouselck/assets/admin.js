...
function ckCallImageManagerPopup(id) {
	CKBox.open({handler: 'iframe', url: 'index.php?option=com_carouselck&view=browse&type=image&func=ckSelectFile&field='+id+'&tmpl=component'});
}

function ckCallVideoManagerPopup(id) {
	CKBox.open({handler: 'iframe', url: 'index.php?option=com_carouselck&view=browse&type=video&func=ckSelectVideo&field='+id+'&tmpl=component'});
}
...
function ckCallMenusSelectionPopup(id) {
	CKBox.open({handler: 'iframe', url: 'index.php?option=com_carouselck&view=menus&fieldid='+id+'&tmpl=component', id:'ckmenusmodal', size: {x: 800, y: 450}});
}
...
//	CKBox.open({handler: 'iframe', url: 'index.php?option=com_content&layout=modal&tmpl=component&task=article.edit&id='+id});
	ckLoadIframeEdition('index.php?option=com_content&layout=modal&tmpl=component&task=article.edit&id='+id, 'carouselckarticleedition', 'article.apply', 'article.cancel', false)
}
...
function ckLoadIframeEdition(url, htmlId, taskApply, taskCancel, close, padding) {
	...
	CKBox.open({id: htmlId, 
				...
				footerHtml: '<a class="ckboxmodal-button" href="javascript:void(0)" onclick="ckSaveIframe(\''+htmlId+'\', ' + close + ')">'+Joomla.JText._('CAROUSELCK_SAVE')+'</a>'
			});
}
...
function ckPreviewStylesparams() {
	...
	var myurl = CAROUSELCK.BASE_URL + "&task=style.ajaxRenderCss&" + CAROUSELCK.TOKEN;
	$ck.ajax({
		type: "POST",
		url: myurl,
		data: {
		...
		var csscode = '<style>' + code.replace(/\|ID\|/g, '#carouselckdemo1') + '</style>';
...
function ckSaveStylesparams(button) {
	...
	customstyles = JSON.stringify(customstyles);
	var myurl = CAROUSELCK.BASE_URL + "&task=style.save&" + CAROUSELCK.TOKEN;
	$ck.ajax({
		type: "POST",
		url: myurl,
		data: {
...
function ckMakeCssReplacement(code) {
	for (var tag in CAROUSELCK.CKCSSREPLACEMENT) {
		var i = 0;
		while (code.indexOf(tag) != -1 && i < 100) {
			code = code.replace(tag, CAROUSELCK.CKCSSREPLACEMENT[tag]);
			i++;
		}
	}
	return code;
...
function ckExportParams() {
	...
	var myurl = CAROUSELCK.BASE_URL + '&task=style.exportParams&' + CAROUSELCK.TOKEN;
	$ck.ajax({
		type: "POST",
		url: myurl,
		async: false,
		...
		if (response == '1') {
			...
			$ck('#ckexportpagedownload').append('<div id="ckexportfile"><a class="btn btn-primary" target="_blank" href="'+CAROUSELCK.URIROOT+'/administrator/components/com_carouselck/export/exportParamsCarouselckStyle'+styleid+'.mmck" download="exportParamsCarouselckStyle'+styleid+'.mmck">'+Joomla.JText._('CK_DOWNLOAD', 'Download')+'</a></div>');
...
function ckUploadParamsFile(formData) {
	var myurl = CAROUSELCK.BASE_URL + '&task=style.uploadParamsFile&' + CAROUSELCK.TOKEN;
	$ck.ajax({
		type: "POST",
		url: myurl,
		async: false,
...
function ckLoadPreset(name) {
	...
	var myurl = CAROUSELCK.BASE_URL + '&task=style.loadPresetFields&' + CAROUSELCK.TOKEN;
	$ck.ajax({
		type: "POST",
		url: myurl,
//		dataType: 'json',
