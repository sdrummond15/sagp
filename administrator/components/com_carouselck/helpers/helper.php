...
require_once JPATH_ADMINISTRATOR . '/components/com_carouselck/helpers/defines.php';

use Carouselck\CKInput;
...
//		$doc->addScript(JUri::root(true) . '/media/jui/js/jquery.min.js');
		$doc->addStyleSheet(CAROUSELCK_MEDIA_URI . '/assets/ckbox.css');
		$doc->addScript(CAROUSELCK_MEDIA_URI . '/assets/ckbox.js');
	}
...
//		$doc = JFactory::getDocument();
//		$doc->addScript(JUri::root(true) . '/media/jui/js/jquery.min.js');
//		$doc->addStyleSheet(CAROUSELCK_MEDIA_URI . '/assets/ckframework.css');
//	}
	...
	?>
		...
		<link rel="stylesheet" href="<?php echo JUri::root(true) ?>/components/com_carouselck/assets/font-awesome.min.css" type="text/css" />
		<link rel="stylesheet" href="<?php echo CAROUSELCK_MEDIA_URI ?>/assets/ckframework.css" type="text/css" />
	<?php
	}*/
	...
	*/
	/*public static function createTableOptions() {
		$sqlsrc = CAROUSELCK_PATH . '/sql/updates/2.4.0.sql';
	...
	public static function getStyleNameById($id) {
		...
		$query->select('a.name');
		$query->from($db->quoteName('#__carouselck_styles') . ' AS a');
	...
	static function resizeImage($file, $x, $y = '', $thumbpath = 'th', $thumbsuffix = '_th') {
		...
		if ($size) {
			...
			if (!JFolder::exists($thumbfolder)) { 
				...
				JFile::copy(JPATH_ROOT . '/modules/mod_carouselck/index.html', $thumbfolder . 'index.html' );
			}
...
//		$articles->setState('params', $appParams);
	...
	public static function getCssReplacement() {
		...
		self::$cssreplacements = Array(
			'[container]' => ''
			,'[slide]' => '.carouselck_images img'
			,'[caption]' => '.carouselck_caption > div'
			,'[title]' => '.carouselck_caption_title'
			,'[text]' => '.carouselck_caption_desc'
			,'[button]' => 'a.carouselck-button'
			,'[buttonhover]' => 'a.carouselck-button:hover'
			,'[commands]' => '|ID| .carouselck_prev, |ID| .carouselck_next, |ID| .carouselck_commands'
			,'[paginationdotthumbs]' => '.carouselck_pag_ul .carouselck_pag_nav img'
		);
...
//		return $css;
	...
	public static function getProPreview($imgName) {
		$html = '<div class="ckpropreview">'
					. '<img src="' . CAROUSELCK_MEDIA_URL . '/images/proonly/' . $imgName . '" />'
					. '<a href="https://www.joomlack.fr/en/joomla-extensions/carousel-ck" target="_blank">' . JText::_('CAROUSELCK_ONLY_PRO') . '</a>'
				. '</div>';
