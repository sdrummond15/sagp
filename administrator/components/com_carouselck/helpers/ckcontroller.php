...
class CKController {
	...
	protected function createView($name, $prefix = '')
	{
		...
		{
			$path = CAROUSELCK_PATH . '/views/' . $this->createFileName('view', array('name' => $viewName));
	...
	public function edit($id = null, $appendUrl = '') {
		...
		CKFof::redirect(CAROUSELCK_ADMIN_URL . '&view=' . $this->getName() . '&layout=edit&id=' . $editId . $appendUrl);
	}
	...
	public function copy() {
		...
		// Redirect to the edit screen.
		CKFof::redirect(CAROUSELCK_ADMIN_URL);
	}
